...
def test_3dm_round_trip_has_units_layers_and_metadata(tmp_path: Path) -> None:
    ...
    plate = {
        ...
        "grid_origin_xyz": [20, 1, 87.02], "grid_u_vector": [0, 2, 0], "grid_v_vector": [-2, 0, 0],
        "plan": {"plan_id": "skyville-dawson-4r-type-a-base", "fixture_digest": "fixture-digest"},
    ...
    write_study_3dm(target, {"digest": "abc", "buildings": [building],
                            "result": {"method_version": "apartment-intelligence-solar-v5", "plate": plate,
                                       "radiation": {"sensor_values_kwh_m2": list(range(12))}},
                            "target": {"building": building, "storey": 30, "facade": "east",
                                       "mirrored": False, "window_width": 4, "window_height": 1.2, "sill_height": .9}})
    ...
    assert model.Strings["method"] == "apartment-intelligence-solar-v5"
    assert len(model.Objects) >= 5
    names = {obj.Attributes.Name for obj in model.Objects}
    assert "Human-confirmed typical apartment floor plate outline" in names
