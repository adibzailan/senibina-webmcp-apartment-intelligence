...
import { Ctx, createStudy, explainEvidence, proposePlacement, runAnalysis, showAnalysis } from "./actions";
...
export function toolDefinitions(ctx: Ctx, supported: any[], exportFromPage: (formats: string[]) => Promise<any>): ToolDef[] {
  ...
  return [
    ...
    { name: "run_solar_analysis", description: "Run the deterministic Ladybug + Radiance analysis. Refused unless the current placement was confirmed by a visible click.", inputSchema: closed({ study_id: { type: "string" }, grid_spacing_m: { type: "number", enum: [0.25, 0.5] } }, ["study_id"]), execute: wrap((i) => { if (i.study_id !== ctx.getState().studyId) throw new Error("404: unknown study_id"); return runAnalysis(ctx, i.grid_spacing_m ?? 0.25); }) },
