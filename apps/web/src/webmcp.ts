...
export function toolDefinitions(ctx: Ctx, supported: any[], exportFromPage: (formats: string[]) => Promise<any>): ToolDef[] {
  ...
  return [
    ...
    { name: "show_analysis", description: "Switch the visible view: analysis sunpath|shadow|solar_access|radiation, date MM-DD, hour, camera precinct|tower|home|plan|north, massing on/off, map on/off (presentation only, read-only). Free orbit, pan and zoom stay with the person.", inputSchema: closed({ analysis: { type: "string", enum: ["sunpath", "shadow", "solar_access", "radiation"] }, date: { type: "string" }, hour: { type: "number" }, camera: { type: "string", enum: ["precinct", "tower", "home", "plan", "north"] }, massing: { type: "boolean" }, map: { type: "boolean" } }), annotations: { readOnlyHint: true }, execute: wrap((i) => showAnalysis(ctx, { ...(i.analysis ? { preset: i.analysis } : {}), ...(i.date ? { date: i.date } : {}), ...(i.hour !== undefined ? { hour: i.hour } : {}), ...(i.camera ? { camera: i.camera } : {}), ...(i.massing !== undefined ? { massing: i.massing } : {}), ...(i.map !== undefined ? { map: i.map } : {}) })) },
