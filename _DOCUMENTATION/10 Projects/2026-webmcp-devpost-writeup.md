...
- How it was made is itself a lesson. Two weeks before the deadline an accident left me with a pin in my wrist, so I could barely type. This project was dictated: I spoke the briefs into Codex, Codex wrote the code, and we went round and round, me reviewing every screen, every number and every export and sending back what was wrong. I read less code and judged more results. I cannot vouch for every line the way I could for a hand-written codebase, and I say so; I can vouch for what it does. Letting the agent run the work while the person keeps the decisions is exactly what the product asks of its own users, and it is a way of working I would not have found without the constraint.
